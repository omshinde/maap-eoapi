...
def test_insert_collection(stac_ingestion_instance, authentication_token, test_collection):
    response = stac_ingestion_instance.insert_collection(authentication_token, test_collection)
...
def test_query_collection(stac_ingestion_instance, test_collection):
    ...
    assert response.json()["id"] == test_collection["id"], "Queried collection does not match the inserted collection"
...
def test_query_items(stac_ingestion_instance, test_collection, test_item):
    ...
    assert any(item["id"] == test_item["id"] for item in items), "Inserted item not found in the queried items"
